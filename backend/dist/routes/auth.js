...
router.post('/register', async (req, res) => {
    ...
    `, [hwid, username]);
        if (isBanned) {
            await db.run('INSERT INTO connection_stats (username, event_type, details) VALUES (?, ?, ?)', [username, 'FAILED_HWID_BAN', `Попытка регистрации с забаненного ПК/Ника: ${isBanned.reason}`]);
        ...
        await db.run('INSERT INTO users (username, password_hash, role, last_hwid) VALUES (?, ?, ?, ?)', [username, passwordHash, 'PLAYER', hwid]);
        await db.run('INSERT INTO connection_stats (username, event_type, details) VALUES (?, ?, ?)', [username, 'REGISTER_SUCCESS', 'Успешная регистрация игрока']);
...
router.post('/login', async (req, res) => {
    ...
    `, [clientHwid, username, clientIp]);
        if (isBanned) {
            await db.run('INSERT INTO connection_stats (username, event_type, details) VALUES (?, ?, ?)', [username, 'FAILED_HWID_BAN', `Попытка входа с забаненного ПК/Ника: ${isBanned.reason}`]);
        ...
        await db.run('INSERT INTO connection_stats (username, event_type, details, ip_address) VALUES (?, ?, ?, ?)', [user.username, 'LOGIN_SUCCESS', `Успешная авторизация (${user.role})`, clientIp]);
        return res.json({
            success: true,
        ...
        return res.status(500).json({ error: 'Внутренняя ошибка сервера при входе: ' + error.message });
    }
});
...
router.get('/verify-session', async (req, res) => {
    try {
        ...
        const db = await (0, db_1.getDb)();
        const session = await db.get('SELECT * FROM sessions WHERE access_token = ? AND LOWER(username) = LOWER(?) AND expires_at > CURRENT_TIMESTAMP', [String(token), String(username)]);
        if (!session) {
...
router.post('/change-password', async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Ошибка при смене пароля: ' + error.message });
    }
});
// ----------------------------------------------------
...
router.post('/discord/request-login', async (req, res) => {
    try {
    ...
    `, [requestId, cleanNick, clientIp, expiresAt]);
        ...
        if (!existingUser) {
            const randomPasswordHash = await bcryptjs_1.default.hash(crypto_1.default.randomBytes(32).toString('hex'), 10);
            await db.run('INSERT INTO users (username, password_hash, role, last_hwid, last_ip) VALUES (?, ?, ?, ?, ?)', [cleanNick, randomPasswordHash, 'PLAYER', clientHwid, clientIp]);
        }
        else {
            await db.run('UPDATE users SET last_hwid = ?, last_ip = ? WHERE id = ?', [clientHwid, clientIp, existingUser.id]);
        }
        await db.run('INSERT INTO connection_stats (username, event_type, details, ip_address, hwid) VALUES (?, ?, ?, ?, ?)', [cleanNick, 'DISCORD_LOGIN_REQUEST', `Запрос авторизации в лаунчер (ID: ${requestId})`, clientIp, clientHwid]);
        // Попытка отправки интерактивного сообщения в Discord
        const dmResult = await (0, discordBot_1.sendDiscordLoginRequest)(cleanNick, clientIp, requestId);
        if (!dmResult.success) {
        ...
        return res.status(500).json({ error: 'Ошибка отправки запроса в Discord: ' + error.message });
    }
});
// GET /api/v1/auth/discord/status/:requestId - Проверка статуса подтверждения запроса
router.get('/discord/status/:requestId', async (req, res) => {
    try {
...
router.post('/discord/callback', async (req, res) => {
    try {
        ...
        if (action === 'approve') {
            ...
            await db.run('INSERT INTO sessions (username, access_token, hwid, ip_address, expires_at) VALUES (?, ?, ?, ?, ?)', [authReq.username, token, 'DISCORD-LAUNCHER', authReq.ip_address || '', expiresAt]);
            await db.run('INSERT INTO connection_stats (username, event_type, details) VALUES (?, ?, ?)', [authReq.username, 'DISCORD_LOGIN_APPROVED', `Вход подтвержден в Discord (${discordId || 'Web'})`]);
...
router.get('/discord/queue', async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
