...
router.post('/report-crash', async (req, res) => {
    try {
        ...
        const mclogsData = await mclogsRes.json();
        return res.json({
            success: true,
...
router.post('/session-ticket', async (req, res) => {
    try {
        ...
        await db.run("INSERT INTO launcher_tickets (id, username, ip_address, expires_at) VALUES (?, ?, ?, datetime('now', '+1 day'))", [ticketId, cleanNick, req.ip || '']);
...
router.get('/verify-ticket', async (req, res) => {
    try {
        ...
        if (!username)
            return res.json({ valid: false, reason: 'Username required' });
        const db = await (0, db_1.getDb)();
        // 5.1. Проверка наличия мобильного байпаса (телефон / PojavLauncher)
        const bypass = await db.get("SELECT * FROM launcher_bypasses WHERE LOWER(username) = LOWER(?) AND (expires_at IS NULL OR expires_at > datetime('now')) LIMIT 1", [username]);
        if (bypass) {
            return res.json({
                valid: true,
                bypass: true,
        ...
        const ticket = await db.get("SELECT * FROM launcher_tickets WHERE LOWER(username) = LOWER(?) AND expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1", [username]);
        if (!ticket) {
...
router.post('/debug-log', async (req, res) => {
    try {
        ...
        if (!log_content)
            return res.status(400).json({ error: 'Log content required' });
        const db = await (0, db_1.getDb)();
        const cleanNick = (username || 'Anonymous').trim();
        await db.run('INSERT INTO launcher_debug_logs (username, os, launcher_version, event_type, log_content) VALUES (?, ?, ?, ?, ?)', [cleanNick, os || 'Unknown OS', launcher_version || '3.1.0', event_type || 'INFO', String(log_content)]);
...
router.post('/crash-report', async (req, res) => {
    try {
        ...
        await db.run('INSERT INTO launcher_crash_reports (username, os, server_id, crash_filename, report_content) VALUES (?, ?, ?, ?, ?)', [cleanNick, os || 'Unknown OS', server_id || 1, crash_filename, String(report_content)]);
