...
async function logAudit(adminUser, adminRole, actionType, targetName, details, ipAddress) {
    try {
        const db = await (0, db_1.getDb)();
        await db.run('INSERT INTO audit_logs (admin_username, admin_role, action_type, target_name, details, ip_address) VALUES (?, ?, ?, ?, ?, ?)', [adminUser, adminRole, actionType, targetName, details, ipAddress]);
    }
    catch (err) {
...
router.get('/me', exports.requireAdmin, async (req, res) => {
    try {
        const user = req.user;
        return res.json({
            authenticated: true,
...
router.get('/admins', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/admins', exports.requireAdmin, async (req, res) => {
    try {
        ...
        if (existing) {
            await db.run("UPDATE users SET password_hash = ?, role = ? WHERE id = ?", [passwordHash, userRole, existing.id]);
        }
        else {
            await db.run("INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)", [cleanNick, passwordHash, userRole]);
        }
        ...
        return res.status(500).json({ error: 'Ошибка добавления администратора: ' + error.message });
    }
});
// DELETE /api/v1/admin/admins/:id - Удаление администратора с немедленным сбросом всех его сессий
router.delete('/admins/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
        ...
        return res.status(500).json({ error: 'Ошибка удаления администратора: ' + error.message });
    }
});
function tcpPingMinecraft(host, port) {
...
router.get('/servers', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/servers', exports.requireAdmin, async (req, res) => {
    try {
        const { name, server_ip, server_port, minecraft_version, neoforge_version, modloader, modloader_version, java_version, jvm_flags, min_ram_gb, recommended_ram_gb, game_args, auto_join_server, description, is_default } = req.body;
        if (!name || !server_ip) {
...
router.put('/servers/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const { name, server_ip, server_port, minecraft_version, neoforge_version, modloader, modloader_version, java_version, jvm_flags, min_ram_gb, recommended_ram_gb, game_args, auto_join_server, description, is_default, is_active } = req.body;
        const db = await (0, db_1.getDb)();
        if (is_default) {
...
router.delete('/servers/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.get('/servers/:id/ping', async (req, res) => {
    try {
        ...
        if (!server)
            return res.status(404).json({ online: false, error: 'Сервер не найден' });
...
router.get('/modpack', exports.requireAdmin, async (req, res) => {
    try {
        const serverId = req.query.serverId ? parseInt(req.query.serverId, 10) : 1;
        const db = await (0, db_1.getDb)();
...
router.post('/modpack/add-modrinth', exports.requireAdmin, async (req, res) => {
    try {
        const { serverId, projectId, versionId } = req.body;
        if (!projectId || !versionId)
            return res.status(400).json({ error: 'projectId и versionId обязательны' });
        const targetServerId = serverId ? parseInt(serverId, 10) : 1;
        const vRes = await fetch(`https://api.modrinth.com/v2/version/${versionId}`);
        if (!vRes.ok)
            return res.status(404).json({ error: 'Версия не найдена на Modrinth' });
        ...
        if (!primaryFile)
            return res.status(400).json({ error: 'У версии нет файлов для скачивания' });
...
router.post('/modpack/check-updates', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const modsStorageDir = path_1.default.resolve(__dirname, '../../public/files/mods');
        const filesDir = path_1.default.resolve(__dirname, '../../public/files');
        const hashesMap = {};
        const hashesList = [];
        for (const mf of modFiles) {
            const cleanName = path_1.default.basename(mf.filepath);
            let localPath = path_1.default.join(modsStorageDir, cleanName);
            if (!fs_1.default.existsSync(localPath)) {
                localPath = path_1.default.join(filesDir, mf.filepath);
            }
            if (fs_1.default.existsSync(localPath)) {
                try {
                    const buf = fs_1.default.readFileSync(localPath);
                    const sha1 = crypto_1.default.createHash('sha1').update(buf).digest('hex');
                    hashesMap[sha1] = {
                        ...mf,
                        localPath,
        ...
        for (let i = 0; i < hashesList.length; i += batchSize) {
            ...
            try {
                ...
                if (updateRes.ok) {
                    ...
                    for (const [h, verObj] of Object.entries(batchData)) {
                        allUpdatesMap[h] = verObj;
                        if (verObj.project_id)
                            projectIdsSet.add(verObj.project_id);
                    }
                }
            }
            catch (err) {
        ...
        return res.status(500).json({ error: 'Ошибка проверки обновлений: ' + error.message });
    }
});
// POST /api/v1/admin/modpack/apply-update - Применение обновления мода из Modrinth
router.post('/modpack/apply-update', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const targetServerId = serverId ? parseInt(serverId, 10) : 1;
        const cleanFilename = path_1.default.basename(newFilename);
        const modsStorageDir = path_1.default.resolve(__dirname, '../../public/files/mods');
        if (!fs_1.default.existsSync(modsStorageDir)) {
            fs_1.default.mkdirSync(modsStorageDir, { recursive: true });
        }
        ...
        const targetFilePath = path_1.default.join(modsStorageDir, cleanFilename);
        ...
        if (oldFilepath) {
            const oldCleanName = path_1.default.basename(oldFilepath);
            if (oldCleanName !== cleanFilename) {
                const oldDiskPath = path_1.default.join(modsStorageDir, oldCleanName);
                if (fs_1.default.existsSync(oldDiskPath)) {
                    try {
                        fs_1.default.unlinkSync(oldDiskPath);
                    }
                    catch (e) { }
                }
            }
    ...
    `, [
        ...
        return res.status(500).json({ error: 'Ошибка применения обновления мода: ' + error.message });
    }
});
...
router.post('/modpack/batch-update-and-zip', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const targetServerId = serverId ? parseInt(serverId, 10) : 1;
        const modsStorageDir = path_1.default.resolve(__dirname, '../../public/files/mods');
        const tempDir = path_1.default.resolve(__dirname, '../../public/files/temp');
        if (!fs_1.default.existsSync(modsStorageDir)) {
            fs_1.default.mkdirSync(modsStorageDir, { recursive: true });
        }
        ...
        const updatedFilesOnDisk = [];
        const errors = [];
        const zip = new adm_zip_1.default();
        for (const item of updates) {
            try {
                if (!item.newFileUrl || !item.newFilename)
                    continue;
                const cleanFilename = path_1.default.basename(item.newFilename);
                const targetFilePath = path_1.default.join(modsStorageDir, cleanFilename);
                ...
                // Удаляем старый файл с диска, если имя файла изменилось
                if (item.oldFilepath) {
                    const oldCleanName = path_1.default.basename(item.oldFilepath);
                    if (oldCleanName !== cleanFilename) {
                        const oldDiskPath = path_1.default.join(modsStorageDir, oldCleanName);
                        if (fs_1.default.existsSync(oldDiskPath)) {
                            try {
        ...
        const zipFilePath = path_1.default.join(tempDir, zipFilename);
        zip.writeZip(zipFilePath);
        const zipStat = fs_1.default.statSync(zipFilePath);
        const adminUser = req.user?.username || 'Admin';
        ...
        return res.status(500).json({ error: 'Ошибка пакетного обновления и архивации: ' + error.message });
    }
});
function normalizeSftpTarget(rawHost, rawPort, rawUsername) {
...
router.post('/modpack/sftp-test-scan', exports.requireAdmin, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (!exists) {
                if (await sftp.exists('/mods'))
                    realPath = '/mods';
                else if (await sftp.exists('./mods'))
                    realPath = './mods';
            }
            ...
            try {
                ...
                throw new Error(`Не удалось прочитать директорию "${targetRemotePath}" на SFTP: ${listErr.message}`);
            }
        }
...
router.post('/modpack/sftp-deploy-updates', exports.requireAdmin, async (req, res) => {
    ...
    for (const item of updates) {
        try {
            ...
            let oldCleanName;
            if (item.oldFilepath) {
                oldCleanName = path_1.default.basename(item.oldFilepath);
                if (oldCleanName !== cleanFilename) {
                    const oldDiskPath = path_1.default.join(modsStorageDir, oldCleanName);
                    if (fs_1.default.existsSync(oldDiskPath)) {
                        try {
...
router.post('/modpack/upload', exports.requireAdmin, async (req, res) => {
    try {
        ...
        if (Array.isArray(allowedUsers)) {
            normUsers = JSON.stringify(allowedUsers.map((u) => u.trim()).filter(Boolean));
        }
...
router.post('/modpack/copy-from-server', exports.requireAdmin, async (req, res) => {
    try {
...
router.delete('/modpack/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.post('/modpack/bulk-delete', exports.requireAdmin, async (req, res) => {
    try {
        const { ids } = req.body;
...
router.post('/modpack/bulk-set-optional', exports.requireAdmin, async (req, res) => {
    try {
...
router.post('/modpack/bulk-edit-details', exports.requireAdmin, async (req, res) => {
    try {
        const { ids, group_name, allowed_users, is_optional } = req.body;
        if (!Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({ error: 'Список ID модов не предоставлен' });
        }
        let normUsers = undefined;
        if (allowed_users !== undefined && allowed_users !== null) {
            if (Array.isArray(allowed_users)) {
                normUsers = JSON.stringify(allowed_users.map((u) => u.trim()).filter(Boolean));
            }
...
router.post('/modpack/group-manager', exports.requireAdmin, async (req, res) => {
    try {
        ...
        if (action === 'set-access') {
            if (!groupName)
                return res.status(400).json({ error: 'Имя группы не указано' });
            let normUsers = 'ALL';
            ...
            await db.run('UPDATE modpack_files SET allowed_users = ? WHERE server_id = ? AND group_name = ?', [normUsers, sId, groupName]);
        ...
        if (action === 'rename-group') {
            ...
            await db.run('UPDATE modpack_files SET group_name = ? WHERE server_id = ? AND group_name = ?', [trimmedNew, sId, groupName]);
        ...
        if (action === 'delete-group') {
            ...
            await db.run("UPDATE modpack_files SET group_name = 'Общие', allowed_users = 'ALL' WHERE server_id = ? AND group_name = ?", [sId, groupName]);
        ...
        if (action === 'set-optional') {
            ...
            await db.run('UPDATE modpack_files SET is_optional = ? WHERE server_id = ? AND group_name = ?', [optVal, sId, groupName]);
        ...
        if (action === 'remove-mods') {
            ...
            const placeholders = modIds.map(() => '?').join(',');
            await db.run(`UPDATE modpack_files SET group_name = 'Общие', allowed_users = 'ALL' WHERE server_id = ? AND id IN (${placeholders})`, [sId, ...modIds]);
...
router.patch('/modpack/:id/toggle-optional', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.patch('/modpack/:id/details', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
...
router.get('/client-servers', exports.requireAdmin, async (req, res) => {
    try {
        const serverId = req.query.serverId ? parseInt(req.query.serverId, 10) : 1;
        const db = await (0, db_1.getDb)();
...
router.post('/client-servers', exports.requireAdmin, async (req, res) => {
    try {
        const { server_id, name, address, icon_base64, is_active } = req.body;
        if (!name || !address)
            return res.status(400).json({ error: 'Название и IP:порт обязательны' });
...
router.put('/client-servers/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
...
router.delete('/client-servers/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.get('/resourcepacks', exports.requireAdmin, async (req, res) => {
    try {
        const serverId = req.query.serverId ? parseInt(req.query.serverId, 10) : 1;
        const db = await (0, db_1.getDb)();
...
router.post('/resourcepacks/upload', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const targetServerId = serverId ? parseInt(serverId, 10) : 1;
        const cleanFilename = path_1.default.basename(filename);
        const rpStorageDir = path_1.default.resolve(__dirname, '../../public/files/resourcepacks');
        if (!fs_1.default.existsSync(rpStorageDir)) {
            fs_1.default.mkdirSync(rpStorageDir, { recursive: true });
        }
        const buffer = Buffer.from(base64Data, 'base64');
        const sha256 = crypto_1.default.createHash('sha256').update(buffer).digest('hex');
        const targetFilePath = path_1.default.join(rpStorageDir, cleanFilename);
        fs_1.default.writeFileSync(targetFilePath, buffer);
...
router.post('/resourcepacks/add-modrinth', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const vRes = await fetch(`https://api.modrinth.com/v2/version/${versionId}`);
        if (!vRes.ok)
            return res.status(404).json({ error: 'Версия ресурспака не найдена на Modrinth' });
        const vData = await vRes.json();
        const pRes = await fetch(`https://api.modrinth.com/v2/project/${projectId}`);
        const pData = pRes.ok ? await pRes.json() : {};
        const primaryFile = vData.files.find((f) => f.primary) || vData.files[0];
...
router.patch('/resourcepacks/:id/details', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
...
router.delete('/resourcepacks/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.get('/bans', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/bans', exports.requireAdmin, async (req, res) => {
    try {
...
router.delete('/bans/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.get('/launcher/releases', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/launcher/upload-release', exports.requireAdmin, async (req, res) => {
    try {
        const { version, release_notes, mac_download_url, win_download_url, linux_download_url, is_mandatory } = req.body;
        if (!version)
            return res.status(400).json({ error: 'Номер версии обязателен (например, 3.1.0)' });
        const db = await (0, db_1.getDb)();
        await db.run(`
...
router.get('/analytics', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.get('/audit-logs', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.get('/neoforge-versions', exports.requireAdmin, async (req, res) => {
    try {
...
router.get('/modrinth/search', exports.requireAdmin, async (req, res) => {
    try {
        const query = req.query.q || '';
        const projectType = (req.query.projectType || 'mod').toLowerCase();
...
router.get('/modrinth/versions', exports.requireAdmin, async (req, res) => {
    try {
...
router.get('/mirrors', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/mirrors', exports.requireAdmin, async (req, res) => {
    try {
        const { name, url, region, priority, is_primary } = req.body;
        if (!name || !url)
            return res.status(400).json({ error: 'Название и URL зеркала обязательны' });
        const db = await (0, db_1.getDb)();
        if (is_primary) {
...
router.post('/mirrors/:id/set-primary', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
        const targetMirror = await db.get("SELECT * FROM api_mirrors WHERE id = ?", [id]);
        if (!targetMirror)
            return res.status(404).json({ error: 'Зеркало не найдено' });
        await db.run("UPDATE api_mirrors SET is_primary = 0");
...
router.delete('/mirrors/:id', exports.requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const db = await (0, db_1.getDb)();
...
router.get('/discord/status', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
        ...
        const maskedToken = rawToken
            ...
            : '';
        const botStatus = (0, discordBot_1.getDiscordBotStatus)();
        return res.json({
            ...botStatus,
            maskedToken,
...
router.post('/discord/config', exports.requireAdmin, async (req, res) => {
    try {
        ...
        await db.run("INSERT INTO system_settings (key, value, updated_at) VALUES ('discord_bot_token', ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at", [cleanToken]);
        if (guildId !== undefined) {
            await db.run("INSERT INTO system_settings (key, value, updated_at) VALUES ('discord_guild_id', ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at", [String(guildId).trim()]);
        }
        if (cleanProxy !== undefined) {
            await db.run("INSERT INTO system_settings (key, value, updated_at) VALUES ('discord_proxy', ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at", [cleanProxy]);
        }
        // Перезапускаем Discord бота на лету с новым токеном и прокси
        const reloadResult = await (0, discordBot_1.reloadDiscordBot)(cleanToken, cleanProxy);
        const adminUser = req.user?.username || 'Admin';
        ...
        if (!reloadResult.success) {
            return res.status(400).json({
                ...
                botStatus: (0, discordBot_1.getDiscordBotStatus)()
            });
        }
        return res.json({
            success: true,
        ...
        return res.status(500).json({ error: 'Ошибка сохранения настроек Discord: ' + error.message });
    }
});
// POST /api/v1/admin/discord/test-dm - Тестовая отправка сообщения в ЛС игроку
router.post('/discord/test-dm', exports.requireAdmin, async (req, res) => {
    try {
        ...
        const cleanNick = username.trim();
        const testRequestId = 'TEST-' + crypto_1.default.randomBytes(6).toString('hex');
        const result = await (0, discordBot_1.sendDiscordLoginRequest)(cleanNick, req.ip || '127.0.0.1', testRequestId);
        if (!result.success) {
        ...
        return res.status(500).json({ error: 'Ошибка отправки теста: ' + error.message });
    }
});
// GET /api/v1/admin/discord/pending-requests - Получение активных запросов на вход игроков
router.get('/discord/pending-requests', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/discord/approve-request', exports.requireAdmin, async (req, res) => {
    try {
        const { requestId } = req.body;
        const db = await (0, db_1.getDb)();
        const authReq = await db.get("SELECT * FROM discord_auth_requests WHERE id = ?", [requestId]);
        if (!authReq)
            return res.status(404).json({ error: 'Запрос на вход не найден или истек' });
        ...
        return res.status(500).json({ error: 'Ошибка одобрения запроса: ' + error.message });
    }
});
// POST /api/v1/admin/discord/simulate-auth - Эмуляция подтверждения авторизации
router.post('/discord/simulate-auth', exports.requireAdmin, async (req, res) => {
    try {
        const { username } = req.body;
        if (!username)
        ...
        const authReq = await db.get("SELECT id FROM discord_auth_requests WHERE LOWER(username) = LOWER(?) AND status = 'PENDING' ORDER BY created_at DESC LIMIT 1", [cleanNick]);
        if (authReq) {
        ...
        const fakeRequestId = 'SIM-' + crypto_1.default.randomBytes(8).toString('hex');
        const expiresAt = new Date(Date.now() + 120 * 1000).toISOString();
        await db.run("INSERT INTO discord_auth_requests (id, username, ip_address, status, expires_at) VALUES (?, ?, '127.0.0.1', 'APPROVED', ?)", [fakeRequestId, cleanNick, expiresAt]);
        return res.json({ success: true, requestId: fakeRequestId, message: `Создана сессия для игрока ${cleanNick}` });
    }
    catch (error) {
        return res.status(500).json({ error: 'Ошибка эмуляции: ' + error.message });
    }
});
// ----------------------------------------------------
...
router.get('/launcher/bypasses', exports.requireAdmin, async (req, res) => {
    try {
        const db = await (0, db_1.getDb)();
...
router.post('/launcher/bypasses', exports.requireAdmin, async (req, res) => {
    try {
    ...
    `, [cleanNick, reason || 'Мобильный клиент (телефон)', adminUser, expiresAt]);
        ...
        return res.status(500).json({ error: 'Ошибка выдачи байпаса: ' + error.message });
    }
});
// DELETE /api/v1/admin/launcher/bypasses/:id - Удалить байпас
router.delete('/launcher/bypasses/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
...
router.get('/debug-logs', exports.requireAdmin, async (req, res) => {
    try {
...
router.get('/debug-logs/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
...
router.get('/debug-logs/:id/download', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
        const logItem = await db.get("SELECT * FROM launcher_debug_logs WHERE id = ?", [id]);
        if (!logItem)
            return res.status(404).send('Лог не найден');
...
router.delete('/debug-logs/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
...
router.get('/crash-reports', exports.requireAdmin, async (req, res) => {
    try {
...
router.get('/crash-reports/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
        const crashItem = await db.get("SELECT * FROM launcher_crash_reports WHERE id = ?", [id]);
        if (!crashItem)
            return res.status(404).json({ error: 'Краш-репорт не найден' });
...
router.get('/crash-reports/:id/download', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
        const crashItem = await db.get("SELECT * FROM launcher_crash_reports WHERE id = ?", [id]);
        if (!crashItem)
            return res.status(404).send('Краш-репорт не найден');
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
...
router.delete('/crash-reports/:id', exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await (0, db_1.getDb)();
