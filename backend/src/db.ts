...
async function initDbSchema(db: Database) {
  ...
  if (!rel356) {
    await db.run(`
      ...
      VALUES ('3.5.6', 'Релиз v3.5.6: Исправление запуска на Windows (ENAMETOOLONG fix), автообновление и оптимизация', 'http://185.221.213.43:3000/files/launchers/VozduCraft-Windows-Setup.exe', 'http://185.221.213.43:3000/files/launchers/VozduCraft-macOS-Setup.dmg', 1, CURRENT_TIMESTAMP)
    `);
  }
