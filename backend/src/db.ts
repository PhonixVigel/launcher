...
async function initDbSchema(db: Database) {
  ...
  await db.run("DELETE FROM launcher_releases WHERE version < '3.5.10'");

  await db.run("UPDATE project_config SET value = '3.5.10' WHERE key = 'launcher_version'");
  const rel359 = await db.get("SELECT id FROM launcher_releases WHERE version = '3.5.10'");
  if (!rel359) {
    await db.run(`
