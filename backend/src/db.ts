...
async function initDbSchema(db: Database) {
  ...
  await db.run("DELETE FROM launcher_releases WHERE version < '3.5.13'");

  await db.run("UPDATE project_config SET value = '3.5.13' WHERE key = 'launcher_version'");
  const rel359 = await db.get("SELECT id FROM launcher_releases WHERE version = '3.5.13'");
  if (!rel359) {
    await db.run(`
      INSERT INTO launcher_releases (version, release_notes, win_download_url, mac_download_url, is_mandatory, created_at)
      VALUES ('3.5.13', '🎉 Тестовое автообновление v3.5.13: Проверка фонового обновления лаунчера', 'http://185.221.213.43:3000/files/launchers/VozduCraft-Windows-Setup.exe?v=3.5.13', 'http://185.221.213.43:3000/files/launchers/VozduCraft-macOS-Setup.dmg?v=3.5.13', 1, CURRENT_TIMESTAMP)
    `);
  } else {
    await db.run(`
      UPDATE launcher_releases 
      SET release_notes = '🎉 Тестовое автообновление v3.5.13: Проверка фонового обновления лаунчера',
          win_download_url = 'http://185.221.213.43:3000/files/launchers/VozduCraft-Windows-Setup.exe?v=3.5.13',
          mac_download_url = 'http://185.221.213.43:3000/files/launchers/VozduCraft-macOS-Setup.dmg?v=3.5.13',
          is_mandatory = 1
