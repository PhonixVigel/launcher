...
async function initDbSchema(db: Database) {
  ...
  // Обязательная регистрация релиза 3.0.9
  const r309 = await db.get("SELECT id FROM launcher_releases WHERE version = '3.0.9'");
  if (!r309) {
    await db.run(`
      INSERT INTO launcher_releases (version, release_notes, mac_download_url, win_download_url, is_mandatory, created_at)
      VALUES 
        ('3.0.9', 'Фикс установки на чистых ПК: загрузка JAR-процессоров инсталлятора', 'http://185.221.213.43:3000/files/launchers/VozduCraft-macOS-Setup.dmg', 'http://185.221.213.43:3000/files/launchers/VozduCraft-Windows-Setup.exe', 1, CURRENT_TIMESTAMP)
    `);
  }
  await db.run("UPDATE project_config SET value = '3.0.9' WHERE key = 'launcher_version'");

  // Системные конфиги
  const defaultConfigs: Record<string, string> = {
    'launcher_version': '3.0.9',
